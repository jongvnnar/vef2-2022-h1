...
describe('users', () => {
  ...
  test('Create user, username too long', async () => {
    ...
    expect(result.errors[0].msg).toEqual(
      'username must be at least 1 character, max 64 characters'
    );
  });
  ...
  test('Create user, too short password', async () => {
    ...
    expect(result.errors.length).toBe(1);
    expect(result.errors[0].msg).toEqual(
      'password must be at least 1 character, max 256 characters'
    );
  });
  ...
  test('Login user, invalid username & pass', async () => {
    ...
    expect(status).toBe(400);
    expect(result.errors.length).toBe(1);
    expect(result.errors[0].msg).toBe('username, email or password incorrect');
  });
  ...
  test('patch /users/me, invalid data', async () => {
    ...
    expect(status).toBe(400);
    expect(result.errors[0].msg).toBe(
      'require at least one value of: name, username, email, password'
    );
  });
